...
function addLayerControl() {
    ...
    var checkOptions2 = {
        ...
        action: function (e) {
            ...
            //    name: 'Hydrolgy'
            //});
            //map.mapTypes.set('Hydrolgy', agsType);
            //checkLayerVisibility('Hydrolgy')
            //map.overlayMapTypes.insertAt(0, agsType);
            //map.overlayMapTypes.visible = false;
            // create the layer
            //var tmsLayer = new TMSLayer(map, "http://services.nationalmap.gov/arcgis/rest/services/nhd/MapServer", true);
            //var tmsLayer = new TMSLayer(map, "https://raster.nationalmap.gov/arcgis/rest/services/LandCover/USGS_EROS_LandCover_NLCD/MapServer/tile/$z/$y/$x", true);
    ...
    var checkOptions4 = {
        ...
        action: function () {
            if (!map.overlayMapTypes.getAt(1)) {
                ...
                map.controls[google.maps.ControlPosition.RIGHT_CENTER].push(myLegendDiv);
            }
            else {
                map.overlayMapTypes.removeAt(1)
                var indexOfControl = null,
                rightCenterControls = map.controls[google.maps.ControlPosition.RIGHT_CENTER];
                rightCenterControls.forEach( function ( element,
                                                         index ) {
...
function checkLayerVisibility(id)
{
    if(map.mapTypes[id] !== 'undefined') { }
    var l = map.mapTypes.overlayMapTypes;
    //if (map.mapTypes.overlayMapTypes.c)
}
function toggleLayerVisibility(layer) {
   
        var overlayMaps = map.overlayMapTypes;
        // find the layer
        for (var i = 0, I = overlayMaps.length; i < I && overlayMaps.getAt(i) != layer; ++i);
       
            // add if the map was not already added
            if (i == overlayMaps.length) {
                overlayMaps.push(layer);
            }
         else {
            // remove if the map was added
            if (i < overlayMaps.length) {
